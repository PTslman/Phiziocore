...
const firebaseConfig = {
    apiKey: "AIzaSyCvXS6nZ2md4xRVyUDtPNOMgL5ShUCADXI",
    authDomain: "semo-7770a.firebaseapp.com",
    projectId: "semo-7770a",
    storageBucket: "semo-7770a.firebasestorage.app",
    messagingSenderId: "1031989206155",
    appId: "1:1031989206155:web:6beaa85c5fd51b5929888e",
    measurementId: "G-4RSY9PYXG3"
};
...
function manualSync() {
    ...
    // إعادة الاستماع للتأكد من التحديث
    patientsRef.get().then(() => {
        showToast('✅ تمت المزامنة بنجاح');
    }).catch(() => {
        showToast('❌ فشلت المزامنة', true);
    });
}
...
async function deletePatient(patientId) {
    showConfirm('حذف مريض', '⚠️ هل أنت متأكد من حذف هذا المريض نهائياً؟', async () => {
        try {
            await patientsRef.doc(patientId).delete();
...
async function addSession() {
    ...
    if (patient) {
        ...
        try {
            await patientsRef.doc(currentPatientId).update({ sessions: newSessions });
...
async function resetSessions() {
    showConfirm('مسح الجلسات', '⚠️ هل تريد مسح جميع جلسات هذا المريض؟', async () => {
        try {
            await patientsRef.doc(currentPatientId).update({ sessions: [] });
    ...
    const html = `
        <div style="direction: rtl; font-family: 'Cairo', sans-serif; max-width: 800px; margin: 0 auto; padding: 30px; background: white;">
            ...
            <div style="background: #f8fafc; border-radius: 16px; padding: 20px; margin-bottom: 25px;">
                <h3 style="color: #1e1b4b; margin-bottom: 15px; display: flex; align-items: center; gap: 8px;">
    ...
    reader.onload = async (e) => {
        try {
            ...
            imported.forEach(pat => {
                const ref = patientsRef.doc(pat.id);
                batch.set(ref, {
                    name: pat.name,
                    price: pat.price,
